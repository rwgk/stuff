...
for z, sym, name in ELEMENTS:
  sym_name_z_table.append((sym, name, z))
  if sym[:1] != name[:1]:
      name_sym_z_table.append((name, sym, z))
sym_name_z_table.sort()
name_sym_z_table.sort()
prev_first = ""
for sym, name, z in sym_name_z_table:
  if sym[:1] != prev_first:
      prev_first = sym[:1]
      print(sym[:1])
  print(z, sym, name)
print()
print("Special")
for name, sym, z in name_sym_z_table:
  print(z, sym, name)
